It is formatted with proper Bash highlighting, callouts, and a logical flow that emphasizes the critical requirement of preserving those Version IDs at the storage level.
...
sudo rsync -avz --progress \
  ...
  <DEST_DATA_PATH>/
3️⃣ Fix Permissions
Ensure the MinIO service user on the destination Ubuntu server owns all the newly migrated directories and files.
...
The rsync command can be safely re-run if the network drops or the process is interrupted.

🧠 Key DevOps Learnings
Storage-Level > API-Level Migration: When strict metadata preservation is required, bypass the API and move the raw backend data.

Metadata is Critical: .minio.sys is the brain of the MinIO deployment.

Idempotency: rsync provides a safe, resumable mechanism for large data transfers.

Service Control: Proper stopping/starting of services is non-negotiable to prevent corruption.

Trust, but Verify: Post-migration validation is what separates a good migration from a failed one.
